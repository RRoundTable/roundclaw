...
// The first turn of an agent's life must create the session; every later turn
// must resume it. Getting this backwards makes turn 2 fail to find the session.
func TestFirstTurnCreatesSessionAndLaterTurnsResume(t *testing.T) {
	env := newEnv(t)
	...
	env.OnActivity("RunClaudeTurn", mock.Anything, mock.Anything).
		Return(func(_ context.Context, in activity.RunTurnInput) (core.TurnResult, error) {
			mu.Lock()
			resumes = append(resumes, in.Resume)
			mu.Unlock()
			// Reporting that the session opened is what makes the next turn
			// resume; a turn counter would advance even when it did not.
	...
	defer mu.Unlock()
	if len(resumes) != 2 {
		t.Fatalf("ran %d turns, want 2", len(resumes))
	}
	if resumes[0] {
		t.Error("first turn passed Resume=true; it must create the session with --session-id")
	}
	if !resumes[1] {
		t.Error("second turn passed Resume=false; it must continue with --resume")
	}
}

// A workflow continued with an established session must resume rather than try
// to create one that already exists.
func TestContinuedWorkflowStillResumes(t *testing.T) {
	env := newEnv(t)
	...
	env.ExecuteWorkflow(SubAgent, Input{AgentID: "test-agent", TurnCount: 42, SessionReady: true})

	mu.Lock()
	defer mu.Unlock()
	if !resume {
		t.Error("a continued workflow with an established session tried to create a new one")
	}
}

// The failure this replaced a turn counter to prevent: a turn that fails before
// the CLI starts must not make the next turn resume a session that was never
// created. Counting turns wedged the agent permanently after one early failure.
func TestFailedFirstTurnDoesNotMakeTheNextOneResume(t *testing.T) {
	env := newEnv(t)
	...
	env.OnActivity("RunClaudeTurn", mock.Anything, mock.Anything).
		Return(func(_ context.Context, in activity.RunTurnInput) (core.TurnResult, error) {
			...
			resumes = append(resumes, in.Resume)
			mu.Unlock()
			// Failed before the CLI ever opened a session.
			return core.TurnResult{
				TurnID: in.TurnID, Status: core.TurnError,
				ErrorMessage: "container would not start", SessionEstablished: false,
			}, nil
		})
	env.OnActivity("DeliverResponse", mock.Anything, mock.Anything).Return(nil)
	...
	for i, r := range resumes {
		if r {
			t.Errorf("turn %d tried to resume a session that was never created", i+1)
		}
	}
}

// A resume that finds nothing must fall back to creating a session, so a lost
// transcript costs the conversation rather than the agent.
func TestLostSessionFallsBackToCreating(t *testing.T) {
	env := newEnv(t)
	...
	env.OnActivity("RunClaudeTurn", mock.Anything, mock.Anything).
		Return(func(_ context.Context, in activity.RunTurnInput) (core.TurnResult, error) {
			...
			resumes = append(resumes, in.Resume)
			mu.Unlock()
			// The resume attempt finds no session, so none is established.
			return core.TurnResult{TurnID: in.TurnID, Status: core.TurnError, SessionEstablished: false}, nil
		})
	env.OnActivity("DeliverResponse", mock.Anything, mock.Anything).Return(nil)
	...
	if !resumes[0] {
		t.Error("the first turn should have attempted a resume")
	}
	if resumes[1] {
		t.Error("after a resume found nothing, the next turn must create a session instead")
	}
}
