...
// TurnCount is cumulative across Continue-As-New, so a workflow resumed with a
// prior turn count must resume rather than try to create the session again.
func TestContinuedWorkflowStillResumes(t *testing.T) {
	...
	env.ExecuteWorkflow(SubAgent, Input{AgentID: "test-agent", TurnCount: 42})

	mu.Lock()
	defer mu.Unlock()
	if !resume {
		t.Error("a workflow continued with TurnCount=42 tried to create a new session")
	}
}
